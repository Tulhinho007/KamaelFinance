...
export default function CartaoDetailErrorPage({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Erro capturado na rota de detalhes do cartão:", error);
  }, [error]);
  ...
  return (
    <div className="min-h-[70vh] p-6 md:p-10 max-w-xl mx-auto flex flex-col items-center justify-center text-center gap-6 select-none animate-in fade-in duration-200">
      ...
      <div className="space-y-2">
        <h2 className="text-xl font-black text-white">Não foi possível carregar a conta ou cartão</h2>
        <p className="text-xs text-slate-400 font-medium max-w-md leading-relaxed mx-auto">
          Ocorreu um erro temporário ao processar as informações desta conta. Tente recarregar ou volte para o módulo de despesas.
        </p>
        {error?.message && (
          <div className="mt-4 p-3.5 bg-slate-900 border border-slate-800 rounded-2xl text-left max-w-md mx-auto">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Detalhes do Erro</span>
